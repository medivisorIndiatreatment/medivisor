...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const description =
    blog.excerpt ||
    "Read informative blog posts about medical treatment, patient journeys, and healthcare tips in India.";
  const image = blog.coverMedia?.image || "/default-blog-og.jpg";
  const url = `https://medivisorindiatreatment.com/blog/${blog.slug}/`;
  const keywords =
    (blog.tags || []).join(", ") ||
    "medical treatment, healthcare, patient stories, Medivisor India";
  ...
  return {
    ...
    keywords,
    alternates: {
      canonical: url, // ✅ Canonical URL added (fixes duplicate issue)
    },
    openGraph: {
      title,
      description,
